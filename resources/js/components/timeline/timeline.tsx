...
function Timeline({ children, section = 'white', date }: Props) {
    ...
    return (
        ...
        >
            <h2
                className={`m-auto w-fit rounded-b-3xl px-6 pt-10 pb-12 text-7xl font-semibold shadow-lg max-sm:m-0 max-sm:mr-auto max-sm:rounded-br-full max-sm:pr-11 max-sm:pl-1 max-sm:text-left sm:rounded-b-3xl ${section === 'blue' ? 'text-primary-blue bg-white' : 'bg-primary-blue text-white'}`}
            >
                {date}
            </h2>
            <ul className={`relative mx-auto grid w-fit grid-cols-4 gap-x-3`}>
                {childrenWithProps}
            </ul>
        </div>
    );
