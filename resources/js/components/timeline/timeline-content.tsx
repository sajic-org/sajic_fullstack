...
function TimelineContent({
    ...
    return (
        <>
            {variant === 'right' ? (
                <li
                    className={`relative col-span-2 col-start-3 ${borderClass} border-l-4 py-6 pl-3`}
                >
                    <h3 className="mb-2 text-4xl font-bold">
                        {date}{' '}
                        <span className="text-base font-normal">({turno})</span>
                    </h3>
                    <div
                        className={`absolute top-9.5 -left-[9.65px] size-4 rounded-full ${bgClass}`}
                    ></div>
                    {children}
                </li>
            ) : (
                <>
                    <li className="col-span-2 py-5 max-sm:hidden">
                        <div className="relative mb-10 text-end">
                            <div
                                className={`absolute top-4 -right-[22.3px] size-4 rounded-full ${bgClass}`}
                            ></div>
                    ...
                    <div
                        className={`${borderClass} border-l-4 max-sm:hidden`}
                    ></div>

                    <li
                        className={`relative col-span-2 col-start-3 ${borderClass} border-l-4 py-6 pl-3 sm:hidden`}
                    >
                        <h3 className="mb-2 text-4xl font-bold">
                            {date}{' '}
